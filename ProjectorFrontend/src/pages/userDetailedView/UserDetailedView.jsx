...
import styles from "./UserDetailedView.module.scss";
...
const UserDetailedView = () => {
  ...
  const [credentialsDisplay, setCredentialsDisplay] = useState(false);
        ...
        const { data, error } = await supabase
          ...
          .select(
            `
              ...
              lastName,
              bio,
              user_qualification ( qualification_id(name) ),
              Education ( eduType, facility, faculty, enrollment, grad ),
              Portfolio ( id, name, role, description )
            `
          )
          .eq("id", UPID)
  ...
  return userInfo ? (
    <div className={styles.detailed}>
      <div className={styles.user_card}>
        <div className={styles.general_info}>
          <div className={cn(styles.img_frame, styles.user_img_frame)}>
            <img
              src={`https://rotyixpntplxytekbeuz.supabase.co/storage/v1/object/public/profile_photos/${userInfo.user_id}/avatar.png`}
          ...
          <div className={styles.user_text}>
            ...
            <h2 className={styles.medium_title3}>
              {userInfo.user_qualification &&
                userInfo.user_qualification[0].qualification_id.name}
            </h2>
          </div>
        </div>
        <div className={styles.credentials}>
          <h2>контакты станут видны после выбора специалиста</h2>
        </div>
      </div>
      <h1>О себе</h1>
      ...
      <div className={styles.project_card}>
        ...
        <p>{userInfo.Education[0].eduType}</p>
        <p>{userInfo.Education[0].facility}</p>
        <p>{userInfo.Education[0].faculty}</p>
        <p>{`${userInfo.Education[0].enrollment} - ${userInfo.Education[0].grad}`}</p>
      </div>
      <h1>Портфолио</h1>
      {userInfo.Portfolio.length > 0 &&
        userInfo.Portfolio.map((item) => (
          <ProjectCard
            key={item.id}
            item={item}
            table={"Portfolio"}
            coverImg={`https://rotyixpntplxytekbeuz.supabase.co/storage/v1/object/public/project_photos/Project_pic.png`}
          />
        ))}
    </div>
  ) : (
