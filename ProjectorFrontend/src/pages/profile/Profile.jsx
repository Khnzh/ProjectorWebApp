...
export default function Profile() {
  ...
  const incrementEduCells = () => {
    if (eduCells < 2) setEduCells((e) => e + 1);
  };
  const decrementEduCells = () => {
    if (eduCells > 0) setEduCells((e) => e - 1);
  };
  const incrementProjectCells = () => {
    if (projectCells < 4) setProjectCells((e) => e + 1);
  };
  const decrementProjectCells = () => {
    if (projectCells > 0) setProjectCells((e) => e - 1);
  };
  ...
  const inputPhNumber = (e) =>
    setProfile((prev) => {
      const { value } = e.target;
      return { ...prev, phNumber: value.trim() };
    });
  const inputSocials = (e) =>
    setProfile((prev) => {
      const { value } = e.target;
  ...
  const inputPrName = (index, event) => {
    ...
    setProject((p) => {
      const updated = p.name.map((name, i) =>
        i === index ? value.trim() : name
      );
  ...
  const inputPrDesc = (index, event) => {
    const { value } = event.target;
    setProject((p) => {
      const updated = p.desc.map((name, i) =>
        i === index ? value.trim() : name
      );
  ...
  const inputPrRole = (index, event) => {
    const { value } = event.target;
    setProject((p) => {
      const updated = p.role.map((name, i) =>
        i === index ? value.trim() : name
      );
  ...
  const inputPrLink = (index, event) => {
    const { value } = event.target;
    setProject((p) => {
      const updated = p.link.map((name, i) =>
        i === index ? value.trim() : name
      );
  ...
  const inputPrYear = (index, event) => {
    const { value } = event.target;
    setProject((p) => {
      const updated = p.year.map((name, i) =>
        i === index ? value.trim() : name
      );
  ...
  const inputMastery = (index, event) => {
    const { value } = event.target;
    setEducation((e) => {
      const updated = e.mastery.map((name, i) =>
        i === index ? value.trim() : name
      );
  ...
  const inputEnrollment = (index, event) => {
    const { value } = event.target;
    setEducation((e) => {
      const updated = e.enrollment.map((name, i) =>
        i === index ? value.trim() : name
      );
  ...
  const inputFacility = (index, event) => {
    const { value } = event.target;
    setEducation((e) => {
      const updated = e.facility.map((name, i) =>
        i === index ? value.trim() : name
      );
  ...
  const inputFaculty = (index, event) => {
    const { value } = event.target;
    setEducation((e) => {
      const updated = e.faculty.map((name, i) =>
        i === index ? value.trim() : name
      );
  ...
  const inputGrad = (index, event) => {
    const { value } = event.target;
    setEducation((e) => {
      const updated = e.grad.map((name, i) =>
        i === index ? value.trim() : name
      );
    ...
    if (error) {
      ...
      for (let i = 0; i < Portfolio.length; i++) {
        for (const key in Portfolio[i]) {
          if (key != "user_id") {
            setProject((p) => {
              const updated = p[key].map((name, index) =>
                index === i ? Portfolio[i][key] : name
              );
              let obj = { ...p };
              obj[key] = updated;
              return obj;
    ...
    switch (active) {
      ...
      case 2:
        ...
        if (hasOnlySpecificStrings(errs, "") || !errs)
          saveProject(project, projectCells, activeProjects);
        break;
      case 3:
        errs = educationValidation(education, eduCells);
        if (hasOnlySpecificStrings(errs, "") || !errs)
          saveEducation(education, eduCells, activeEdu);
        break;
    }
    setErrors(errs);
  };
    ...
    <>
      <div className={styles.tab_container}>
        ...
        >
          ...
          />
          {errors.socials && (
            <p className="validation-message">{errors.socials}</p>
          )}
        </div>
        ...
        >
          ...
          {education.eduType.map((edu, index) => (
            <div
              className={styles.additional_edu}
              style={{
                display:
                  index == 0 ? "flex" : eduCells >= index ? "flex" : "none",
              }}
            >
              ...
              <div className={styles.radio_no_underline}>
                {edu == "general" ? (
                  <input
                    onClick={(e) => inputEduType(index, e)}
                    disabled={mode}
                    type="radio"
                    id={index + "general"}
                    name={index + "eduType"}
                ...
                <label htmlFor={index + "general"}>
                  <svg
                    className={"radio sign__svg " + styles.radio__svg}
                    viewBox="0 0 40 38"
                  >
                    <path
                      className="box"
                      d="M31.7,2.1C23.3,2.3,14.9,3.2,6.4,3.6C3.6,3.8,1.3,6.3,1.5,9.1c2.1,32.2-4.1,26.5,20.9,26.5
                  ...
                  <span className={styles.radio__label}>Основное</span>
                </label>
                {edu == "additional" ? (
                  <input
                    onClick={(e) => inputEduType(index, e)}
                ...
                <label htmlFor={index + "additional"}>
                  <svg
                    className={"radio sign__svg " + styles.radio__svg}
                    viewBox="0 0 40 38"
                  >
                  ...
                  <span className={styles.radio__label}>Дополнительное</span>
                </label>
              </div>
              ...
              <input
                onChange={(e) => inputFacility(index, e)}
                disabled={mode}
                type="text"
                name="edFacility"
                id={index + "edFacility"}
                defaultValue={education.facility[index]}
                placeholder="Введите..."
              />
              ...
              <input
                onChange={(e) => inputFaculty(index, e)}
                disabled={mode}
                type="text"
                name="faculty"
                id={index + "faculty"}
                defaultValue={education.faculty[index]}
                placeholder="Введите..."
              />
              ...
              <input
                onChange={(e) => inputMastery(index, e)}
                disabled={mode}
                type="text"
                name="mastery"
                id={index + "mastery"}
                defaultValue={education.mastery[index]}
                placeholder="Введите..."
              />
              ...
              <input
                onChange={(e) => inputEnrollment(index, e)}
                disabled={mode}
                type="number"
                name="enYear"
                id={index + "enYear"}
                defaultValue={education.enrollment[index]}
                placeholder="Введите..."
              />
              ...
              <input
                onChange={(e) => inputGrad(index, e)}
                disabled={mode}
                type="number"
                name="gradYear"
                id={index + "gradYear"}
                defaultValue={education.grad[index]}
                placeholder="Введите..."
              />
          ...
          {mode == 0 && (
            <p className={styles.add} onClick={incrementEduCells}>
              Добавить
            </p>
          )}
        </div>
        ...
        >
          ...
          {project.name.map((name, index) => (
            <div
              ...
              style={{
                display:
                  index == 0 ? "flex" : projectCells >= index ? "flex" : "none",
              }}
            >
              <div className={styles.edu_header}>
                ...
                {index == 0 ? (
                  ...
                  mode == 0 && (
                    <p
                      className={styles.delete_cell}
                      onClick={() => deleteRow("Portfolio", project, o)}
                    >
                      delete
                    </p>
                  )
                  ...
                  <button
                    onClick={decrementProjectCells}
                    className={styles.section_deletion}
                  ></button>
                )}
              </div>
              <input
                onChange={(e) => inputPrName(index, e)}
                disabled={mode}
                type="text"
                name="prName"
                id={index + "prName"}
              ...
              <textarea
                onChange={(e) => inputPrDesc(index, e)}
                disabled={mode}
                name="prDesc"
                id={index + "prDesc"}
                defaultValue={project.desc[index]}
              ></textarea>
              ...
              <input
                onChange={(e) => inputPrRole(index, e)}
                disabled={mode}
                type="text"
                name="prRole"
                id={index + "prRole"}
                defaultValue={project.role[index]}
              />
              ...
              <input
                onChange={(e) => inputPrLink(index, e)}
                disabled={mode}
                type="text"
                name="prLink"
                id={index + "prLink"}
                defaultValue={project.link[index]}
              />
              ...
              <input
                onChange={(e) => inputPrYear(index, e)}
                disabled={mode}
                type="number"
                name="prYear"
                id={index + "prYear"}
                defaultValue={project.year[index]}
              />
