...
const UserCard = ({ item, coverImg, index }) => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      const cardPosition = document.getElementById(`user-${item.id}`).getBoundingClientRect();
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(styles.img_frame, styles.user_img_frame)}
        style={coverUserImage(coverImg)}
      >
        <img
      ...
      <div className={styles.user_text}>
        ...
        <h1 className={styles.title4}>{item.name} {item.lastName} </h1>
        <h1 className={styles.title5}>{item.user_qualification.length!=0 && `(${item.user_qualification[0].qualifications.name})`}</h1>
        </div>
