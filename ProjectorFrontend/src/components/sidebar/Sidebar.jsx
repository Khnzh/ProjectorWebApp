...
export default function Sidebar({sb}) {
  ...
  useEffect(() => {
    ...
    const fetchAvatarURL = () => {
      ...
      if (uId){
        if (!localStorage.getItem('name') || !localStorage.getItem('lastName')) {fetchName(uId)}
        else {
          setName(localStorage.getItem('name'));
          setLastName(localStorage.getItem('lastName'));
        }
        if (!localStorage.getItem('specs')) {fetchSpecs()}
        else {
          setSpecs(JSON.parse(localStorage.getItem('specs')));
          setMainSpec(JSON.parse(localStorage.getItem('specs'))[0])
        }
        if (!localStorage.getItem('avatar')) {fetchAvatarURL()}
        else {setAvatarURL(localStorage.getItem('avatar'))}
      }
  ...
  return (
    <div className={cn(styles.root, styles.sidebar)} ref={sb}>
      <div className={styles.flex_col}>
        ...
        <h1 className={styles.big_title}><span className={styles.no_wrap}>{name + ' ' + lastName}</span></h1>
        <h2 className={styles.medium_title}>{email}</h2>
        
        <div className={styles.flex_row_info}>
          {specs && <DropdownSpecs specs={specs} mainSpec={mainSpec} setMainSpec={setMainSpec} />}
          <img
        ...
        <h2 className={styles.medium_title1}><span className={styles.no_wrap}>Редактировать профиль</span></h2>
        ...
        <div className={styles.flex_row}>
          <img className={styles.image17} src={'/assets/e2eb7164ddaa335cc3d6e67781eeceda.svg'} alt="alt text" />
          <a className={styles.title1}>КАТАЛОГ</a>
        </div>

        <div className={styles.flex_row1}>
          <div className={styles.flex_row_info}>
          <img className={styles.image18} src={'/assets/9453e8c218b9d080e60d081dcc90771c.svg'} alt="alt text" />
        ...
        <div className={styles.flex_row1}>
          <img className={styles.image19} src={'/assets/6fad9b44cb0964bb4ba8adad213b29c2.svg'} alt="alt text" />
          <a className={styles.title1}>СПЕЦИАЛИСТЫ</a>
        </div>

        <div className={styles.flex_row1}>
          <img className={styles.image20} src={'/assets/34278ddd79993269b66817f2d5d5ed8e.svg'} alt="alt text" />
